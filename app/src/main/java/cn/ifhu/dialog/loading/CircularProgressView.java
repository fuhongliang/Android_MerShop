package cn.ifhu.dialog.loading;
...
public class CircularProgressView extends View {
    ...
    private ArrayList<AnimatorSet> mWaitCleanAnimatorSetList = new ArrayList<>();
    ...
    public void setProgress(final float currentProgress) {
        ...
        if (!isIndeterminate) {
            ...
            progressAnimator.addListener(new SafeAnimatorListenerAdapter() {
                @Override
    ...
    private void resetAnimation() {
        ...
        if (!isIndeterminate) {
            ...
            startAngleRotate.addListener(new SafeAnimatorListenerAdapter());
        ...
        // Indeterminate animation
            ...
            mWaitCleanAnimatorSetList.add(indeterminateAnimator);
            ...
            indeterminateAnimator.addListener(new SafeAnimatorListenerAdapter() {
    ...
    private AnimatorSet createIndeterminateAnimator(float step) {
        ...
        frontEndExtend.addListener(new SafeAnimatorListenerAdapter());
        ...
        rotateAnimator1.addListener(new SafeAnimatorListenerAdapter());
        ...
        backEndRetract.addListener(new SafeAnimatorListenerAdapter());
        ...
        rotateAnimator2.addListener(new SafeAnimatorListenerAdapter());

        AnimatorSet set = new AnimatorSet();
        set.play(frontEndExtend).with(rotateAnimator1);
        set.play(backEndRetract).with(rotateAnimator2).after(rotateAnimator1);
        set.addListener(new SafeAnimatorListenerAdapter());
        return set;
    }

    private void cancelAnimatorSet() {
        Log.d(TAG, "cancelAnimatorSet listSize = " + mWaitCleanAnimatorSetList.size());
        for (AnimatorSet animatorSet : mWaitCleanAnimatorSetList) {
            if (animatorSet != null) {
                animatorSet.removeAllListeners();
                animatorSet.cancel();
                mWaitCleanAnimatorSetList.remove(animatorSet);
            }
        }
    }
